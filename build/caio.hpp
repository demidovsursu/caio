...
struct lexinit_node : Lrule_domain {
	List<Code> f1_;
	lexinit_node(List<Code> a1, const YYLTYPE *_loc=nullptr):Lrule_domain(_loc), f1_(a1){}
	~lexinit_node() {
	  destroy(f1_);
	}
};
...
inline Lrule lexinit(List<Code> a1, const YYLTYPE *_loc=nullptr) {
	return new lexinit_node(a1, _loc);
}
...
union YYSTYPE {
  ...
  Xrule f14_;
  ast::Value<std::string> f1_;
  ast::Value<std::string> f12_;
};
template <typename T> inline T& YYSTYPE_cast(YYSTYPE &v){ return *reinterpret_cast<T*>(&v); }
